    ...
    <ListPageLayout rootDir={DASHBOARD} title="Dashboard">
      <SimpleGrid cols={3} breakpoints={[{ maxWidth: 'md', cols: 1 }]}>
        <TotalBox
          ...
          loading={false}
          icon={
            <AiIcons.AiFillDollarCircle className={styles.totalSalesIcon} />
          }
        />
        <TotalBox
          title="Total Orders"
          amount={300000}
          loading={false}
          icon={<FaIcons.FaTruckMoving className={styles.totalOrdersIcon} />}
        />
        <TotalBox
          title="Total Products"
          loading={isValidating}
          amount={product?.length}
          icon={<FaIcons.FaShoppingBag className={styles.totalProductsIcon} />}
        />
      </SimpleGrid>
      <Grid gutter="md">
        <Grid.Col md={6} lg={8}>
